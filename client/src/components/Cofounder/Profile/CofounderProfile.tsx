...
function CofounderProfile() {
  const [cofounderDetails, setCofounderDetails] =
    useState<CofounderRegisterPayload>();
  useEffect(() => {
    const cofounderDetails = async () => {
          ...
          <div className="flex  items-center gap-6 ">
            <div className="mt-8 ml-8 mb-16 grid justify-items-start">
              <Typography variant="h2" color="white" className="mb-1">
                {cofounderDetails?.companyName ?? ""}
              </Typography>
              <Typography variant="h5" className="font-normal text-white">
                {cofounderDetails?.location ?? ""}
              </Typography>
              <Typography variant="h6" className="font-normal text-white">
                {cofounderDetails?.about ?? ""}
              </Typography>
              {/* <Typography
                  variant="small"
                  className="font-normal text-blue-gray-600"
                >
                  {cofounderDetails?.industry ?? ""}
                </Typography> */}
            </div>
            {/* <Avatar
              src={cofounderDetails?.image ?? ""}
              alt="img"
              size="xl"
          ...
          <div className="gird-cols-1 mb-12 grid gap-4 px-4 lg:grid-cols-2 xl:grid-cols-1">
            <Typography variant="h5" color="blue-gray" className="mb-3">
              <div className="flex gap-x-3">
                Thông tin cá nhân
                <Link to={"/cofounder/edit-profile"}>
