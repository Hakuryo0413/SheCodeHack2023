...
export const jobApplicationDbRepository = (
  ...
  const jobApplicationsForCofounder = async (cofounderId: string) => {
    const applications = await repository.jobApplicationForCofounder(
      cofounderId
    );
    return applications;
  };
  ...
  const changeApplicationStatus = async (
    ...
    const updatedApplication = await repository.changeApplicationStatus(
      jobId,
      status
    );
